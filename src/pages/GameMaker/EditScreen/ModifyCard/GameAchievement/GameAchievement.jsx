...
const ids = [0,1,2,3,4]
export default class GameAchievement extends Component {
    ...
    render() {
        ...
        const {message} = this.props.gameachievement
        // const changeNumberValue = (event) => {
        //     const {textDatas} = this.state
        //     textDatas.gameachievement.score[Number(event.target.id)] = Number(event.target.value)
        //     PubSub.publishSync("setFormDatas",{name: this.props.name, values: textDatas})
        // };
        
        const changeTextValue = (event) => {
            const {textDatas} = this.state
            textDatas.gameachievement.message[Number(event.currentTarget.id)] = event.target.value
            PubSub.publishSync("setFormDatas",{name: this.props.name, values: textDatas})
        }
                        ...
                        ids.map((id)=>{
                            return (
                                <div key={id}>
                                    <Row>
                                        <Col span={24} >
                                            <Input value={id === 0 ? "當物品1接中較多時" : id === 1 ? "當物品2接中較多時"  : id === 2 ? "當物品3接中較多時" 
                                                 : id === 3 ? "當物品4接中較多時"  : "當物品5接中較多時"}  disabled/>
                                        </Col>
